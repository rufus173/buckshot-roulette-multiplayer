...
class gui():
    def __init__(self) -> None:#just gonna initialise the entire gui
        ...
        self.player_2_frame = tkinter.LabelFrame(self.root,text="player 2",bg="grey")
        self.player_2_frame.grid(row=0,column=0,columnspan=1,rowspan=1,sticky=tkinter.NSEW)
        self.root.columnconfigure(0,weight=1)
        self.player_2_items = {}
        for i in range(8):
            self.player_2_items[i] = tkinter.Button(self.player_2_frame,fg="grey")
        for i in range(4):
        ...
        self.shotgun_frame = tkinter.LabelFrame(self.root,text="shotgun",bg="grey")
        self.shotgun_frame.grid(row=0,column=1,columnspan=1,rowspan=1,sticky=tkinter.NSEW)
        self.player_2_select = tkinter.Button(self.shotgun_frame,text="you",fg="grey")
